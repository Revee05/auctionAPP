    ...
    (e) => {
      ...
      const doDrag = (dragEvent) => {
        const newWidth = Math.max(
          minWidth,
          startWidth + (dragEvent.clientX - startX)
        );
        onResize(newWidth);
      };
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between h-full">
        {children}
        <div
          className={`absolute right-0 top-0 bottom-0 w-4 cursor-col-resize flex items-center justify-center group touch-none z-10 ${
            isResizing ? "bg-blue-500/20" : "hover:bg-blue-500/10"
          }`}
          onMouseDown={startResize}
        >
          <div
            className={`w-0.5 h-4 bg-zinc-300 dark:bg-zinc-700 group-hover:bg-blue-500 transition-colors ${
...
export default function UserManagement() {
  ...
  const handleAssignRole = async (userId, roleName) => {
    ...
    try {
      ...
      for (const r of existingRoles) {
        ...
        try {
          await apiClient.delete(`/api/superadmin/users/${userId}/roles`, {
            data: { roleName: r },
          });
        } catch (e) {
          console.warn("Failed to remove role", r);
        }
      }

      // Add new role
      if (!existingRoles.includes(roleName)) {
        await apiClient.post(`/api/superadmin/users/${userId}/roles`, {
          roleName,
        });
      }
      ...
      setError(err.response?.data?.error || "Failed to assign role"); // Use page error state
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleDelete = async (userId, userRoles = []) => {
    if (userRoles.includes("SUPER_ADMIN"))
      return alert("Cannot delete a SUPER_ADMIN account");
  ...
  const handleBan = async (userId, userRoles = []) => {
    if (userId === currentUser?.id) return alert("Cannot ban yourself");
    if (userRoles.includes("SUPER_ADMIN"))
      return alert("Cannot ban a SUPER_ADMIN");
  ...
  const handleUnban = async (userId, userRoles = []) => {
    ...
    try {
      const endpoint = isSuperAdmin
        ? `/api/superadmin/users/${userId}/unban`
    ...
    <div className="space-y-6">
          ...
          <div className="flex gap-3 overflow-x-auto pb-1 max-w-full">
            <div className="flex items-center gap-3 px-4 py-2 bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm whitespace-nowrap">
              ...
              <div>
                ...
                <p className="font-bold">{totals.totalUsers ?? "-"}</p>
              </div>
            </div>
            <div className="flex items-center gap-3 px-4 py-2 bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm whitespace-nowrap">
              ...
              <div>
                ...
                <p className="font-bold">{totals.activeUsers ?? "-"}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="w-full relative bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 overflow-hidden shadow-sm">
              ...
              <thead>
                <tr>
                  ...
                  <ResizableHeader
                    ...
                    minWidth={200}
                    onResize={(w) => handleResize("user", w)}
                  >
                    User
                  </ResizableHeader>
                  <ResizableHeader
                    ...
                    minWidth={120}
                    onResize={(w) => handleResize("role", w)}
                  >
                    Role
                  </ResizableHeader>
                  <ResizableHeader
                  ...
                  <ResizableHeader
                    width={colWidths.joined}
                    minWidth={120}
                    onResize={(w) => handleResize("joined", w)}
                  >
                    Joined
                  </ResizableHeader>
                  {(isSuperAdmin || isAdmin) && (
                    ...
                    >
                      <span className="text-right block w-full">Actions</span>
                    </ResizableHeader>
                  )}
                </tr>
              </thead>
              <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
                {filteredUsers.length === 0 ? (
                  ...
                  filteredUsers.map((user) => (
                    ...
                    >
                      <td
                        className="p-4 text-xs font-mono text-zinc-500 overflow-hidden text-ellipsis whitespace-nowrap"
                        style={{ maxWidth: colWidths.id }}
                      >
                        #{user.id}
                      </td>
                            ...
                            <DropdownMenu>
                              <DropdownMenuTrigger asChild>
                                <Button
                                  variant="ghost"
                                  size="icon"
                                  className="h-6 w-6 text-zinc-400 hover:text-blue-500 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-full"
                                >
                      ...
                      <td
                        className="p-4"
                        style={{ maxWidth: colWidths.status }}
                      >
                        <span
                          className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium border ${getStatusBadgeClasses(
                            user.status
                          )}`}
                        >
                          <span
                      ...
                      <td
                        ...
                        style={{ maxWidth: colWidths.joined }}
                      >
                      ...
                      {(isSuperAdmin || isAdmin) && (
                        <td
                          ...
                          style={{ maxWidth: colWidths.actions }}
                        >
                          <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                              <Button
                            ...
                            >
                              ...
                              {user.status === "banned" ? (
                                <DropdownMenuItem
                                  onClick={() =>
                                    handleUnban(user.id, user.roles)
                                  }
                                >
                              ...
                              {isSuperAdmin && (
                                <>
                                  ...
                                  <DropdownMenuItem
                                    onClick={() =>
                                      handleDelete(user.id, user.roles)
                                    }
                                    className="text-red-600 focus:text-red-600 target:text-red-600"
                                  >
        ...
        <div className="flex items-center justify-between p-4 border-t border-zinc-200 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-900/50">
          <p className="text-sm text-zinc-500">Page {currentPageIndex + 1}</p>
          <div className="flex items-center gap-2">
            <Button
              variant="outline"
              size="sm"
